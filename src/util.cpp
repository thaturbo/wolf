...
const char * const WOLF_CONF_FILENAME = "wolf.conf";
const char * const WOLF_PID_FILENAME = "wolfd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "wolf" is a composite category enabling all WOLF-related debug output
            if(ptrCategory->count(string("wolf"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\WOLFCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\WOLFCore
    // Mac: ~/Library/Application Support/WOLFCore
    // Unix: ~/.wolfcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WOLFCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/WOLFCore";
#else
    // Unix
    return pathRet / ".wolfcore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", WOLF_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty wolf.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override wolf.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", WOLF_PID_FILENAME));
