...
const std::string WOLFGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString WOLFGUI::DEFAULT_WALLET = "~Default";

WOLFGUI::WOLFGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QPixmap pixmapLogo(":/images/light/wolf_logo_horizontal");
...
void WOLFGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a WOLF address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and wolf: URIs)"));
    ...
    aboutAction = new QAction(tr("&About WOLF"), this);
    aboutAction->setStatusTip(tr("Show information about WOLF"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for WOLF"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your WOLF addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified WOLF addresses"));
    ...
    openAction->setStatusTip(tr("Open a wolf: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the WOLF help message to get a list with possible WOLF command-line options"));
...
void WOLFGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void WOLFGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool WOLFGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool WOLFGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void WOLFGUI::setWalletActionsEnabled(bool enabled)
{
...
void WOLFGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("WOLF client") + " " + networkStyle->getTitleAddText();
...
void WOLFGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void WOLFGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void WOLFGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void WOLFGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void WOLFGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void WOLFGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void WOLFGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void WOLFGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void WOLFGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void WOLFGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void WOLFGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void WOLFGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to WOLF network", "", count));
    } else {
...
void WOLFGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void WOLFGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void WOLFGUI::updateHeadersSyncProgressLabel()
{
...
void WOLFGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void WOLFGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void WOLFGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("WOLF"); // default title
...
void WOLFGUI::changeEvent(QEvent *e)
{
...
void WOLFGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void WOLFGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void WOLFGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(WOLFUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void WOLFGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void WOLFGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool WOLFGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool WOLFGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void WOLFGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void WOLFGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void WOLFGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void WOLFGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void WOLFGUI::detectShutdown()
{
...
void WOLFGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void WOLFGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void WOLFGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(WOLFGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void WOLFGUI::subscribeToCoreSignals()
{
...
void WOLFGUI::unsubscribeFromCoreSignals()
{
...
void WOLFGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void WOLFGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<WOLFUnits::Unit> units = WOLFUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const WOLFUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(WOLFUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(WOLFUnits::Unit u, WOLFUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(WOLFUnits::name(u)), this);
...
{
    setText(WOLFUnits::name(newUnits));
}
