    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("WOLF Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About WOLF Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  wolf-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_WOLF_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", WOLFGUI::DEFAULT_UIPLATFORM));
        }
...
All the WOLF in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 WOLF, 0.1 WOLF, 1 WOLF and 10 WOLF -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("WOLF Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(WOLFGUI *window)
{
    if (!window)
        return nullptr;
