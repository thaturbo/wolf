...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "WOLFNetwork") type = WOLFNetwork;
    if(strType == "WOLFNetworkVariable") type = WOLFNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == WOLFNetwork) s = "WOLFNetwork";
    if(type == WOLFNetworkVariable) s = "WOLFNetworkVariable";
