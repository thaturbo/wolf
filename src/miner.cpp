...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in wolf, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in WOLF
//
...
// ***TODO*** that part changed in wolf, we are using a mix with old one here for now
void static WOLFMiner(const CChainParams& chainparams, CConnman& connman)
{
    LogPrintf("WOLFMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wolf-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("WOLFMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("WOLFMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("WOLFMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("WOLFMiner -- runtime error: %s\n", e.what());
        return;
    }
}

void GenerateWOLFs(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WOLFMiner, boost::cref(chainparams), boost::ref(connman)));
}
