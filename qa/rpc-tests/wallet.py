...
from test_framework.test_framework import WOLFTestFramework
from test_framework.util import *

class WalletTest (WOLFTestFramework):
    ...
    def check_fee_amount(self, curr_balance, balance_with_fee, fee_per_byte, tx_size):
        ...
        if fee < target_fee:
            raise AssertionError("Fee of %s WOLF too low! (Should be %s WOLF)"%(str(fee), str(target_fee)))
        # allow the node's estimation to be at most 2 bytes off
        if fee > fee_per_byte * (tx_size + 2):
            raise AssertionError("Fee of %s WOLF too high! (Should be %s WOLF)"%(str(fee), str(target_fee)))
        return curr_balance
    ...
    def run_test (self):
        ...
        # Send 210 WOLF from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 1000 WOLF in block rewards plus fees, but
        ...
        # Send 100 WOLF with subtract fee from amount
        ...
        # Sendmany 100 WOLF with subtract fee from amount
