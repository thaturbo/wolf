...
###   This script attempts to download the signature file SHA256SUMS.asc from wolf.org
...
RCSUBDIR="test/"
BASEDIR="https://wolf.org/bin/"
VERSIONPREFIX="wolf-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'wolf-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the WOLF Core binary release signing key installed?"
   fi
