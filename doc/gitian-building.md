...
*Setup instructions for a Gitian build of WOLF Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the WOLF
...
- [Building WOLF Core](#building-wolf-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-wolf-host/README.md).
...
Clone the git repositories for WOLF Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/wolfcrypto/wolf
```
...
in the WOLF Core repository under 'Fetch and build inputs' to install sources which require
...
To build WOLF Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the WOLF Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/wolf/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/wolfcrypto/wolf
...
For example:
```bash
URL=https://github.com/crowning-/wolf.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit wolf=${COMMIT} --url wolf=${URL} ../wolf/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit wolf=${COMMIT} --url wolf=${URL} ../wolf/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit wolf=${COMMIT} --url wolf=${URL} ../wolf/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the wolf git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../wolf/contrib/gitian-descriptors/*|sort|uniq )
...
WOLFPATH=/some/root/path/wolf.git
SIGPATH=/some/root/path/wolf-detached-sigs.git

./bin/gbuild --url wolf=${WOLFPATH},signature=${SIGPATH} ../wolf/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/wolf-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/wolf-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/wolf-osx-build.assert
```
...
[wolf/gitian.sigs](https://github.com/wolfcrypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
